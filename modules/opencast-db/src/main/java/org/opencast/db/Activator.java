...
public class Activator implements BundleActivator {
  ...
  public void start(BundleContext bundleContext) throws Exception {
    ...
    rootDir = bundleContext.getProperty("org.opencast.storage.dir") + File.separator + "db";

    // Register the data source, defaulting to an embedded H2 database if DB configurations are not specified
    String jdbcDriver = getConfigProperty(bundleContext.getProperty("org.opencast.db.jdbc.driver"),
            "org.h2.Driver");
    String jdbcUrl = getConfigProperty(bundleContext.getProperty("org.opencast.db.jdbc.url"),
            "jdbc:h2:" + rootDir);
    String jdbcUser = getConfigProperty(bundleContext.getProperty("org.opencast.db.jdbc.user"), "sa");
    String jdbcPass = getConfigProperty(bundleContext.getProperty("org.opencast.db.jdbc.pass"), "sa");

    Integer maxPoolSize = getConfigProperty(bundleContext.getProperty("org.opencast.db.jdbc.pool.max.size"));
    Integer minPoolSize = getConfigProperty(bundleContext.getProperty("org.opencast.db.jdbc.pool.min.size"));
    Integer acquireIncrement = getConfigProperty(
            bundleContext.getProperty("org.opencast.db.jdbc.pool.acquire.increment"));
    Integer maxStatements = getConfigProperty(
            bundleContext.getProperty("org.opencast.db.jdbc.pool.max.statements"));
    Integer loginTimeout = getConfigProperty(
            bundleContext.getProperty("org.opencast.db.jdbc.pool.login.timeout"));
    Integer maxIdleTime = getConfigProperty(
            bundleContext.getProperty("org.opencast.db.jdbc.pool.max.idle.time"));
    Integer maxConnectionAge = getConfigProperty(
            bundleContext.getProperty("org.opencast.db.jdbc.pool.max.connection.age"));

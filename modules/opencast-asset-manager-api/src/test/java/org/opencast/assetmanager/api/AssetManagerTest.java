...
package org.opencast.assetmanager.api;
...
public class AssetManagerTest {
  ...
  public void testAssetManagerApiCompilation() {
    ...
    // Select the latest version of "mp-id" only if it has properties of namespace "org.opencast.service" attached.
    // Fetch only the media package and associated metadata like version, creation date etc.
    q.select(q.snapshot()).where(q.mediaPackageId("mp-id").and(q.version().isLatest()).and(q.hasPropertiesOf("org.opencast.service"))).run();
    ...
    // Select all media packages and all of their versions that have boolean property "org.opencast.myservice:approved" set to true.
    // Fetch media packages and associated metadata like version, creation date etc.
    // Fetch all properties of namespace "org.opencast.myservice"
    q.select(q.snapshot(), q.propertiesOf("org.opencast.myservice"))
            .where(q.property(Value.BOOLEAN, "org.opencast.myservice", "approved").eq(true)).run();

    // The above works but it's better to define and use a property schema!
    final Props p = new Props(q);

    // Select all versions of media packages that have boolean property "org.opencast.myservice:approved" set to true.
    q.select(q.snapshot()).where(p.approved().eq(true)).run();

    // Select all versions of media packages and properties of namespace "org.opencast.myservice" having a "count" property of less than 10.
    ...
    // Select properties of namespace "org.opencast.myservice" of media package "mp-id".
    q.select(p.allProperties()).where(q.mediaPackageId("mp-id")).run();

    // Select property "org.opencast.myservice:count" of media package "mp-id".
    ...
    // Delete all properties of namespace "org.opencast.myservice" of all media packages.
    q.delete("owner", q.propertiesOf("org.opencast.myservice")).run();

    // Delete all properties of namespace "org.opencast.myservice" from media package "mp-id".
    q.delete("owner", q.propertiesOf("org.opencast.myservice")).where(q.mediaPackageId("mp-id")).run();

    // Delete property "org.opencast.myservice:prop" from media package "mp-id".
    q.delete("owner", q.properties(PropertyName.mk("org.opencast.myservice", "prop"))).where(q.mediaPackageId("mp-id")).run();

    // Delete properties "org.opencast.myservice:prop" and "org.opencast.myservice:prop2" from media package "mp-id".
    q.delete("owner", q.properties(PropertyName.mk("org.opencast.myservice", "prop"), PropertyName.mk("org.opencast.myservice", "prop2"))).where(q.mediaPackageId("mp-id")).run();

    // Delete all "org.opencast.myservice:approved" properties from all media package having a "org.opencast.myservice:count" greater 10.
    ...
    // Set property "org.opencast.myservice:count" of media package "mp-id" to 10.
    am.setProperty(Property.mk(PropertyId.mk("mp-id", "org.opencast.myservice", "count"), Value.mk(10L)));

    // Set property "org.opencast.myservice:count" of media package "mp-id" to 10 using the schema definition.
  ...
  static class Props extends PropertySchema {
    Props(AQueryBuilder q) {
      super(q, "org.opencast.myservice");
    }

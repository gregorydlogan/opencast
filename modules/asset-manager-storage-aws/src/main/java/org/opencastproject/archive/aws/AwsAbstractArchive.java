...
import org.opencastproject.archive.aws.persistence.AwsArchiveDatabase;
...
public abstract class AwsAbstractArchive implements AssetStore {
  ...
  protected AwsArchiveDatabase database;
  ...
  /** OSGi Di */
  public void setDatabase(AwsArchiveDatabase db) {
    this.database = db;
  }
  ...
  {
    try {
      AwsArchiveMapping map = database.findMapping(from);
      if (map == null) {
  ...
  public boolean contains(StoragePath path) throws AssetStoreException {
    try {
      AwsArchiveMapping map = database.findMapping(path);
      return (map != null);
    } catch (AwsArchiveDatabaseException e) {
      throw new AssetStoreException(e);
    }
  ...
  protected abstract InputStream getObject(AwsArchiveMapping map) throws AssetStoreException;
  ...
  public boolean delete(DeletionSelector sel) throws AssetStoreException {
    ...
    try {
      List<AwsArchiveMapping> list = database.findMappingsByMediaPackageAndVersion(path);
      // Traverse all file mappings for that media package / version(s)
      for (AwsArchiveMapping map : list) {
        // Find all mappings that point to the same object (like hard-links)
        List<AwsArchiveMapping> links = database.findMappingsByKey(map.getObjectKey());
        if (links.size() == 1) {
  ...
  protected abstract void deleteObject(AwsArchiveMapping map) throws AssetStoreException;
}

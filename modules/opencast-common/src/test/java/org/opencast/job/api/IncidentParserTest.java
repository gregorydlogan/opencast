...
import static org.opencast.fun.juc.Immutables.map;
import static org.opencast.util.IoSupport.loadFileFromClassPathAsString;
...
public class IncidentParserTest {
  ...
  public void testSerializationOfJaxbIncident() throws Exception {
    ...
    assertThat(the(marshaled),
            similarTo(the(loadFileFromClassPathAsString("/org/opencast/job/api/expected-incident-1.xml").get())));
  ...
  public void testSerializationOfJaxbIncidentTree() throws Exception {
    ...
    logger.info(marshaled);
    assertThat(the(marshaled),
            similarTo(the(loadFileFromClassPathAsString("/org/opencast/job/api/expected-incident-tree-1.xml")
                    .get())));

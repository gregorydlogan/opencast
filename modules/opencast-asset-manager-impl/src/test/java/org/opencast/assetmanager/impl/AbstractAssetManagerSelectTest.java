...
import static org.opencast.assetmanager.api.fn.ARecords.getProperties;
import static org.opencast.assetmanager.api.fn.ARecords.getSnapshot;
...
import org.opencast.assetmanager.api.fn.Snapshots;
...
public class AbstractAssetManagerSelectTest extends AbstractAssetManagerTestBase {
  ...
  public void testSelectProperties() throws Exception {
    ...
    //
    assertEquals("No records should be found", 0, q.select(q.snapshot()).where(q.hasPropertiesOf("org.opencast.service")).run().getSize());
    //
    logger.info("Set property on first episode");
    am.setProperty(Property.mk(PropertyId.mk(mp1.getIdentifier().toString(), "org.opencast.service", "count"), Value.mk(10L)));
    assertEquals("One record should be found", 1, q.select(q.snapshot()).where(q.hasPropertiesOf("org.opencast.service")).run().getSize());
    //
  ...
  public void testSelectMultiplePropertyNamespaces() throws Exception {
    ...
    //
    am.setProperty(Property.mk(PropertyId.mk(mp1Id, "org.opencast.scheduler", "start"), Value.mk(new Date())));
    am.setProperty(Property.mk(PropertyId.mk(mp1Id, "org.opencast.scheduler", "end"), Value.mk(new Date())));
    //
    {
      final RichAResult r = enrich(q.select(p.allProperties(), q.propertiesOf("org.opencast.scheduler")).run());
    ...
    {
      final RichAResult r = enrich(q.select(q.propertiesOf("org.opencast.scheduler")).where(q.mediaPackageId(mp1Id)).run());
  ...
  public void testSelectMultiplePropertyNamespaces_CERV_695() throws Exception {
    ...
    //
    am.setProperty(Property.mk(PropertyId.mk(mp[0], "org.opencast.scheduler", "start"), Value.mk(new Date())));
    am.setProperty(Property.mk(PropertyId.mk(mp[0], "org.opencast.scheduler", "end"), Value.mk(new Date())));
    am.setProperty(Property.mk(PropertyId.mk(mp[0], "org.opencast.scheduler", "enabled"), Value.mk(true)));
    //
    am.setProperty(Property.mk(PropertyId.mk(mp[0], "org.opencast.annotation", "annotation"), Value.mk("some text")));
    //
    am.setProperty(Property.mk(PropertyId.mk(mp[0], "org.opencast.distribution", "channel"), Value.mk("engage")));
    {
      final RichAResult r = enrich(q.select(
              q.snapshot(),
              p.count.target(), p.approved.target(),
              q.propertiesOf("org.opencast.annotation", "org.opencast.scheduler"),
      ...
      assertEquals("There should be 3 properties in the 'scheduler' namespace",
                   3, sizeOf(r.getProperties().filter(Properties.byNamespace("org.opencast.scheduler"))));
      assertEquals("There should be 1 property in the 'annotation' namespace",
                   1, sizeOf(r.getProperties().filter(Properties.byNamespace("org.opencast.annotation"))));
    ...
    }
    {
      final RichAResult r = enrich(
              q.select(
                      ...
                      p.count.target(),
                      p.approved.target(),
                      q.propertiesOf("org.opencast.annotation", "org.opencast.scheduler"), p.legacyId.target())
                      .where(

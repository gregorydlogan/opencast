...
import org.opencast.util.persistence.PersistenceEnvs;
...
public class OsgiIncidentServiceTest {
  ...
  public void testRetrieving() throws Exception {
    ...
    expect(job.getJobType()).andStubReturn("org.opencast.service");
    expect(job.getCreator()).andStubReturn("creator");
    expect(job.getOrganization()).andStubReturn("organization");
    replay(job);
    ...
    assertEquals("org.opencast.service", incidents.get(0).getServiceType());
    assertEquals("org.opencast.service.1511", incidents.get(0).getCode());
    // todo more tests
  }

  @Test
  public void testGenDbKeys() {
    assertEquals(Immutables.list("org.opencast.composer.1.title.de.DE",
            "org.opencast.composer.1.title.de", "org.opencast.composer.1.title"),
            OsgiIncidentService.genDbKeys(OsgiIncidentService.localeToList(Locale.GERMANY),
                    "org.opencast.composer.1.title"));
    assertEquals(Immutables.list("org.opencast.composer.1.title"),
            OsgiIncidentService.genDbKeys(Immutables.<String> nil(), "org.opencast.composer.1.title"));
  }
}
